...
# The list of valid campus codes, from <https://www.jcu.edu.au/__data/assets/pdf_file/0003/202899/TT-Information-for-Students.pdf>
CAMPUS_CODES = {
...
class TimetableScraper:
	...
	def __init__(self):
		...
		self._session.headers.update({'User-Agent': 'Mozilla/5.0 (Linux) Gecko/20100101 Firefox/43.0'})
	...
	# Extracts the string from an element that specifies the date ranges for an event
	...
	def scrape(self, filters, campus, sp):
		...
		matches = self._match_subjects(subjects, self._transform_filters(filters, sp, campus))
		...
		timetableData = self._session.post(MAIN_PAGE, data=params, headers=headers)
		...
		columnLabels = list(th.text for th in parsedPage.select('.cyon_table thead th'))
		...
		for row in tableRows:
			...
			for index, td in enumerate(row.select('td')):
				...
				key = columnLabels[index].replace('(', '').replace(')', '').replace(' ', '_').lower()
...
def requestHandler():
	try:
		...
		if 'subjects' not in request.args or 'campus' not in request.args or 'sp' not in request.args:
			raise Exception('The following arguments are required: "subjects", "campus", "sp"')
		...
		if campus.upper() not in CAMPUS_CODES:
			message = 'Invalid Campus specified. Valid values are: ' + ', '.join((code + ' (' + CAMPUS_CODES[code] + ')') for code in CAMPUS_CODES)
			raise Exception(message)
		
		# Check that the specified Study Period is valid
		if sp not in WEEKS_MAPPING:
			raise Exception('Invalid Study Period specified. An integer between 1 and 11 must be provided.')
		
		# Perform the timetable scrape
		scraper = TimetableScraper()
		return json.dumps(scraper.scrape(filters, campus, sp), sort_keys=True, indent=4)
